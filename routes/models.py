...
def digest_visits(date_start, date_end) -> List[Tuple[str, str]]:
    visits: List[Visit] = Visit.objects.filter(
        delivery_date__gte=date_start, delivery_date__lt=date_end
    )
...
def update_relationships() -> None:
    ...
    for (driver, member_id), quantity in counts.items():
        if ship := RelationshipCache.objects.filter(driver=driver, member_id=member_id):
            ship.update(cache_date=date_end, visit_count=F("visit_count") + quantity)
        else:
            RelationshipCache.objects.create(
                cache_date=date_end,
                driver=driver,
                member_id=member_id,
